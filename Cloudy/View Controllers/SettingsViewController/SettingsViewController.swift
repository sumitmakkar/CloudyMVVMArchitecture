...
extension SettingsViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        switch section
        {
            ...
            case .units:
                guard let unitNotation = UnitsNotation(rawValue: indexPath.row)
                else
                {
                    fatalError("Unexpected Index Path")
                }
                viewModel = SettingsViewUnitsViewModel(unitsNotation: unitNotation)
            case .temperature:
                guard let tempratureNotation = TemperatureNotation(rawValue: indexPath.row)
                else
                {
                    fatalError("Unexpected Index Path")
                }
                viewModel = SettingsViewTemperatureViewModel(tempratureNotation: tempratureNotation)
        }
